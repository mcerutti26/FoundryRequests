...
Outside of the Lambda functions defined in the template file, the Slack application uses several AWS resources including StepFunctions, 
...
```bash
FoundryRequests$ sam build --use-container
```
...
```bash
FoundryRequests$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
FoundryRequests$ sam local start-api
FoundryRequests$ curl http://localhost:3000/
```
...
```bash
FoundryRequests$ sam logs -n HelloWorldFunction --stack-name "foundryrequests" --tail
```
...
```bash
FoundryRequests$ pip install -r tests/requirements.txt --user
# unit test
FoundryRequests$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
FoundryRequests$ AWS_SAM_STACK_NAME="foundryrequests" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "foundryrequests"
```
